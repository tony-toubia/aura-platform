@echo off
REM Script to set environment variables for Aura Platform on Google Cloud Run

echo Setting environment variables for Aura Dashboard...

REM Load environment variables from .env.production in parent directory
for /f "tokens=1,2 delims==" %%a in (..\\.env.production) do (
    if not "%%a"=="" if not "%%b"=="" set "%%a=%%b"
)

REM Set environment variables for the dashboard/web app
...
REM Set environment variables for the marketing site
